...
const ChakraCarousel = ({ children, gap, slidePerView = 4 }) => {
  ...
  useEffect(() => {
    ...
    if (isBetweenMdAndXl) {
      setItemWidth(sliderWidth / Math.min(3, slidePerView) - gap);
      setMultiplier(0.5);
      setConstraint(Math.min(4, slidePerView));
    }
    if (isGreaterThanXL) {
      setItemWidth(sliderWidth / Math.min(4, slidePerView) - gap);
      setMultiplier(0.35);
      setConstraint(Math.min(4, slidePerView));
    }
